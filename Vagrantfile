...
Vagrant.configure("2") do |config|
    ...
    config.vm.provision "shell", path: "provisioning/base-provision.sh", args: "vpn"
    config.vm.provision "shell", path: "provisioning/config-provision.sh", args: "vpn"
    config.vm.provision "shell", path: "provisioning/ros-provision.sh"
